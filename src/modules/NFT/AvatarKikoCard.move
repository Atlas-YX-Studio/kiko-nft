...
module AvatarKikoCard {
    ...
    fun get_element_by_id(sender: &signer, nft_id: u64, type_id: u64): Option<NFT<ElementMeta, ElementBody>> {
        ...
        assert(AvatarKikoElement::get_type_id(nft) == type_id, TYPE_NOT_MATCH);
        return option_nft
    }
    ...
    fun get_score(option_nft: &Option<NFT<ElementMeta, ElementBody>>): u128 {
        if (Option::is_some<NFT<ElementMeta, ElementBody>>(option_nft)) {
            let nft = Option::borrow<NFT<ElementMeta, ElementBody>>(option_nft);
            return AvatarKikoElement::get_score(nft)
        };
        return 0
    }
    ...
    public fun composite_original_card_from_external(
        sender: &signer,
        name: vector<u8>,
        image: vector<u8>,
        description: vector<u8>,
        ...
        gifts_id: u64,
        tail_id: u64
    ): NFT<KikoCatMeta, KikoCatBody> acquires KikoCatNFTCapability, KikoCatGallery, ElementGallery {
    ...
    public(script) fun composite_original_card_v2(
        sender: signer,
        name: vector<u8>,
        image: vector<u8>,
        description: vector<u8>,
        ...
        assert(NFT_ADDRESS == sender_, PERMISSION_DENIED);
        let metadata = NFT::new_meta_with_image(name, image, description);
        let nft = composit_nft(
            &sender,
            metadata,
